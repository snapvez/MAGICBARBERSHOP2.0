...
-- Consolidate appointments policies
...
CREATE POLICY "appointments_select_policy" ON appointments FOR SELECT TO authenticated USING (auth.uid() = client_id OR auth.uid() = guest_id OR barber_id IN (SELECT id FROM barbers WHERE auth_user_id = auth.uid()) OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = appointments.barber_id))));
CREATE POLICY "appointments_insert_policy" ON appointments FOR INSERT TO authenticated WITH CHECK (auth.uid() = client_id OR auth.uid() = guest_id OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = appointments.barber_id))));
CREATE POLICY "appointments_update_policy" ON appointments FOR UPDATE TO authenticated USING (auth.uid() = client_id OR auth.uid() = guest_id OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = appointments.barber_id)))) WITH CHECK (auth.uid() = client_id OR auth.uid() = guest_id OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = appointments.barber_id))));
CREATE POLICY "appointments_delete_policy" ON appointments FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = appointments.barber_id))));

-- Consolidate other table policies
...
CREATE POLICY "barbers_select_policy" ON barbers FOR SELECT TO authenticated USING (is_active = true OR auth_user_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid()));
CREATE POLICY "barbers_update_policy" ON barbers FOR UPDATE TO authenticated USING (auth_user_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid())) WITH CHECK (auth_user_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid()));
...
CREATE POLICY "client_subscriptions_select_policy" ON client_subscriptions FOR SELECT TO authenticated USING (client_id = auth.uid() OR guest_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = client_subscriptions.barber_id))));
CREATE POLICY "client_subscriptions_insert_policy" ON client_subscriptions FOR INSERT TO authenticated WITH CHECK (client_id = auth.uid() OR guest_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = client_subscriptions.barber_id))));
CREATE POLICY "client_subscriptions_update_policy" ON client_subscriptions FOR UPDATE TO authenticated USING (client_id = auth.uid() OR guest_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = client_subscriptions.barber_id)))) WITH CHECK (client_id = auth.uid() OR guest_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = client_subscriptions.barber_id))));
CREATE POLICY "client_subscriptions_delete_policy" ON client_subscriptions FOR DELETE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND (admin_users.role = 'super_admin' OR (admin_users.role = 'barber_admin' AND admin_users.barber_id = client_subscriptions.barber_id))));
...
CREATE POLICY "commission_settings_select_policy" ON commission_settings FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND admin_users.role = 'super_admin'));
CREATE POLICY "commission_settings_update_policy" ON commission_settings FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND admin_users.role = 'super_admin')) WITH CHECK (EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid() AND admin_users.role = 'super_admin'));
...
CREATE POLICY "loyalty_rewards_select_policy" ON loyalty_rewards FOR SELECT TO authenticated USING (is_active = true OR EXISTS (SELECT 1 FROM admin_users WHERE admin_users.auth_user_id = auth.uid()));
...
SELECT b.id as barber_id, b.name as barber_name, DATE_TRUNC('month', a.appointment_date) as month, COUNT(*) as total_appointments, SUM(s.price) as total_revenue, SUM(COALESCE(bp.points_earned, 0)) as total_points, (SELECT commission_percentage FROM commission_settings LIMIT 1) as commission_rate
FROM appointments a JOIN barbers b ON a.barber_id = b.id JOIN services s ON a.service_id = s.id LEFT JOIN barber_points bp ON bp.appointment_id = a.id WHERE a.status = 'completed' GROUP BY b.id, b.name, DATE_TRUNC('month', a.appointment_date);

DROP VIEW IF EXISTS revenue_breakdown;
CREATE VIEW revenue_breakdown AS
SELECT DATE_TRUNC('month', a.appointment_date) as month, COUNT(*) as total_appointments, SUM(s.price) as total_revenue, SUM(CASE WHEN cs.id IS NOT NULL THEN s.price ELSE 0 END) as subscription_revenue, SUM(CASE WHEN cs.id IS NULL THEN s.price ELSE 0 END) as individual_revenue, COUNT(DISTINCT a.barber_id) as active_barbers
FROM appointments a JOIN services s ON a.service_id = s.id LEFT JOIN client_subscriptions cs ON a.client_id = cs.client_id WHERE a.status = 'completed' GROUP BY DATE_TRUNC('month', a.appointment_date);
