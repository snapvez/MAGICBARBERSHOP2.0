...
import { Calendar, X, Plus, CheckCircle, Clock, User, Phone, ChevronLeft, ChevronRight } from 'lucide-react';
...
export const AdminCalendar = ({ onNavigate }: AdminCalendarProps) => {
  ...
  const handleConfirm = async () => {
    ...
    if (confirmAction.type === 'confirm') {
      await executeConfirmAppointment(confirmAction.id);
    } else if (confirmAction.type === 'complete') {
      await executeCompleteAppointment(confirmAction.id);
    } else {
                                    ...
                                    <div className="flex gap-1">
                                      {appointment.status === 'pending' && (
                                        <button
                                          ...
                                          className="px-2 py-0.5 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs font-semibold transition-all"
                                          title="Confirmar"
                                        >
                                          ✓
                                        </button>
                                      ...
                                      {appointment.status === 'confirmed' && (
                                        <button
                                          ...
                                          className="px-2 py-0.5 bg-green-600 hover:bg-green-700 text-white rounded text-xs font-semibold transition-all"
                                          title="Concluir"
                                        >
                                          ✓✓
                                        </button>
                                      ...
                                      <button
                                        ...
                                        className="px-2 py-0.5 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-semibold transition-all"
                                        title="Cancelar"
                                      >
                                        ✗
                                      </button>
            ...
            <div className="text-center">
              <div className="mb-6">
                ...
                <p className="text-magic-yellow text-lg">
                  {confirmAction.type === 'confirm'
                    ? 'Tem a certeza que quer confirmar esta marcação? O cliente será notificado.'
                    : confirmAction.type === 'complete'
                    ? 'Tem a certeza que quer marcar esta marcação como concluída?'
