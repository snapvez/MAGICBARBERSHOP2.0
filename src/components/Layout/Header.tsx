import { Phone, LogIn, LogOut, Shield, Zap, Scissors } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          >
            {!logoError ? (
              <img
                src="/logo.jpeg"
                alt="Magic Barbershop Logo"
                className="h-10 sm:h-16 w-auto object-contain"
                onError={() => setLogoError(true)}
              />
            ) : (
              <div className="h-10 sm:h-16 w-10 sm:w-16 bg-magic-gold/20 rounded-lg flex items-center justify-center">
                <Scissors className="h-6 sm:h-10 w-6 sm:w-10 text-magic-gold" />
              </div>
            )}
            <div className="text-left">
              <h1 className="text-sm sm:text-xl md:text-2xl font-bold text-magic-gold tracking-wide">MAGIC BARBERSHOP</h1>
            ...
            {user ? (
              <>
                <div className="hidden sm:block text-right">
                  ...
                  <p className="text-xs text-magic-yellow">{isAdmin ? 'Administrador' : 'Cliente'}</p>
                </div>
                <button
