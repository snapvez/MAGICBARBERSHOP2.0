  ...
  const loadBookedTimes = async () => {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      for (let minute = 0; minute < 60; minute += 15) {
        ...
        let availableBarberCount = barbers.length;
        ...
        availableBarberCount = barbers.filter(b => !busyBarbers.has(b.id)).length;
  ...
  const handleBooking = async () => {
    ...
    if (error) {
      ...
      if (error.message.includes('marcação de assinatura pendente') ||
        ...
        errorMessage = 'Já tens uma marcação de assinatura pendente. Aguarda a confirmação do administrador antes de fazer nova marcação.';
        if (hasValidSubscription) {
          await checkPendingAppointments();
        }
